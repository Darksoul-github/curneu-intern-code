...
std::string DES::Triple_DES_Decrypt(std::string plaintext){	
	// PC1 table
    ...
    std::reverse(RKB_3.begin(),RKB_3.end());
    std::reverse(RKH_3.begin(),RKH_3.end());
    std::string cipher_final = Encrypt_Tool(plaintext,RKB_3,RKH_3);

     //ENCRYPTION WITH KEY_2
    ...
    std::string cipher_2 = Encrypt_Tool(cipher_final,RKB_2,RKH_2);
    ...
    std::reverse(RKB_1.begin(),RKB_1.end());
    std::reverse(RKH_1.begin(),RKH_1.end());
    std::string cipher = Encrypt_Tool(cipher_2,RKB_1,RKH_1);
    return cipher;
}
